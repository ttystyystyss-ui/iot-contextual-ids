...
ARTIFACT_DIR = Path("artifacts/rolling_quantile_gnn_gru_ids_run")
...
class RollingMultiStepGATGRU(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        raw = self.head(torch.cat([hidden, context], dim=1))
        return self.monotonic_quantiles(raw)

    def monotonic_quantiles(self, raw):
        raw = raw.view(raw.shape[0], len(PHYSICAL_NODES), len(QUANTILES))
        center = raw[:, :, 0]
        lower_width = torch.nn.functional.softplus(raw[:, :, 1])
        upper_width = torch.nn.functional.softplus(raw[:, :, 2])
        quantiles = torch.stack(
            [center - lower_width, center, center + upper_width],
            dim=-1,
        )
        return quantiles.reshape(raw.shape[0], len(PHYSICAL_NODES) * len(QUANTILES))
...
def quantile_loss(prediction, target, weights):
    batch_size = target.shape[0]
    quantile_count = len(QUANTILES)
    prediction = prediction.view(batch_size, len(PHYSICAL_NODES), quantile_count)
    ...
    return loss.sum() / (weights.sum().clamp_min(1e-8) * quantile_count)


def interval_errors(prediction, target, weights):
    batch_size = target.shape[0]
    prediction = prediction.view(batch_size, len(PHYSICAL_NODES), len(QUANTILES))
    lower = prediction[:, :, 0]
    upper = prediction[:, :, -1]
    low_violation = (lower - target).clamp_min(0.0)
    high_violation = (target - upper).clamp_min(0.0)
    violation = low_violation + high_violation
    per_target = (violation ** 2) * weights
    per_sample = per_target.sum(dim=1) / weights.sum(dim=1).clamp_min(1e-8)
    return per_sample, per_target
...
def evaluate(model, loader, device, scalers):
    ...
    with torch.no_grad():
        for x, context, y, weights in loader:
            ...
            loss = quantile_loss(pred, y, weights)
            losses.append(loss.item())
            per_sample, per_target = interval_errors(pred, y, weights)
...
def write_prediction_details(path, samples, predictions, targets, response_scalers):
    ...
    for name in PHYSICAL_NODES:
        fields.extend([
            f"actual_response_{name}",
            f"pred_response_{name}_p10",
            f"pred_response_{name}_p50",
            f"pred_response_{name}_p90",
            f"interval_err_{name}",
        ])
    with open(path, "w", newline="", encoding="utf-8") as file:
        ...
        for sample, pred_seq, target_seq in zip(samples, predictions, targets):
            ...
            for index, name in enumerate(PHYSICAL_NODES):
                q_start = index * len(QUANTILES)
                quantiles = [
                    pred_seq[q_start + q_index]
                    for q_index in range(len(QUANTILES))
                ]
                raw_quantiles = [
                    response_scalers[name].inverse_transform(value)
                    for value in quantiles
                ]
                lower = quantiles[0]
                upper = quantiles[-1]
                actual = target_seq[index]
                violation = max(lower - actual, 0.0) + max(actual - upper, 0.0)
                row[f"actual_response_{name}"] = raw_target[index]
                row[f"pred_response_{name}_p10"] = raw_quantiles[0]
                row[f"pred_response_{name}_p50"] = raw_quantiles[1]
                row[f"pred_response_{name}_p90"] = raw_quantiles[2]
                row[f"interval_err_{name}"] = violation ** 2
            writer.writerow(row)
...
def train(args):
    ...
    checkpoint = {
        "model_type": "rolling_quantile_gat_gru_delta_predictor",
        "prediction_target": "tail_median_response_quantiles",
        ...
        "error_method": "tail_median_response_quantile_interval",
        "response_tail_start_index": tail_start_index(args.prediction_horizon_seconds),
        "state_dict": model.state_dict(),
    }
    model_path = output_dir / "rolling_quantile_gnn_gru_ids.pt"
    ...
    print(f"[INFO] Error method: tail_median_response_quantile_interval start_step={tail_start_index(args.prediction_horizon_seconds) + 1}")
    print(f"[INFO] Quantiles: {QUANTILES}")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train rolling quantile GAT-GRU IDS without temperature.")
