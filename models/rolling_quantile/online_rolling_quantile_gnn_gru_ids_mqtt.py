...
from train_rolling_quantile_gnn_gru_ids import (
    NODE_NAMES,
    PHYSICAL_NODES,
    QUANTILES,
...
MODEL_PATH = "artifacts/rolling_quantile_gnn_gru_ids_run/rolling_quantile_gnn_gru_ids.pt"
...
ALERT_TOPIC = "iot/rolling_quantile_gnn_gru_ids_alert"
ONLINE_LOG = Path("artifacts/rolling_quantile_gnn_gru_ids_online_log.csv")
POLL_INTERVAL = 1.0
...
LOG_FIELDS = [
    ...
    "pred_ambient_light",
    "pred_total_power_w",
    "pred_ambient_light_p10",
    "pred_ambient_light_p50",
    "pred_ambient_light_p90",
    "pred_total_power_w_p10",
    "pred_total_power_w_p50",
    "pred_total_power_w_p90",
    "actual_delta_ambient_light",
    "actual_delta_total_power_w",
    "pred_delta_ambient_light",
    "pred_delta_total_power_w",
    "pred_delta_ambient_light_p10",
    "pred_delta_ambient_light_p50",
    "pred_delta_ambient_light_p90",
    "pred_delta_total_power_w_p10",
    "pred_delta_total_power_w_p50",
    "pred_delta_total_power_w_p90",
...
class OnlineRollingMultiStepIDS:
    def __init__(self, checkpoint, args):
        ...
        self.model = RollingMultiStepGATGRU(
            ...
            target_size=len(PHYSICAL_NODES) * len(QUANTILES),
    ...
    def start_prediction(self, current_index):
        ...
        with torch.no_grad():
            prediction = self.model(x, context).view(1, len(PHYSICAL_NODES), len(QUANTILES))
        ...
        metadata = {
            ...
            "prediction": prediction[0].reshape(-1).detach().cpu().tolist(),
        }
    ...
    def score_prediction(self, pending):
        ...
        prediction = prediction.view(1, len(PHYSICAL_NODES), len(QUANTILES))
        ...
        lower = prediction[:, :, 0]
        upper = prediction[:, :, -1]
        low_violation = (lower - target).clamp_min(0.0)
        high_violation = (target - upper).clamp_min(0.0)
        violation = low_violation + high_violation
        per_target_errors = (violation ** 2) * weights
        ...
        predicted_response_quantiles = {}
        predicted_response = {}
        actual_response = {}
        predicted_tail_values = {}
        predicted_tail_quantiles = {}
        actual_tail_values = {}
        for index, name in enumerate(PHYSICAL_NODES):
            predicted_response_quantiles[name] = [
                self.response_scalers[name].inverse_transform(value)
                for value in prediction[0, index].detach().cpu().tolist()
            ]
            predicted_response[name] = predicted_response_quantiles[name][1]
            actual_response[name] = raw_actual_response[index]
            predicted_tail_values[name] = previous_record[name] + predicted_response[name]
            predicted_tail_quantiles[name] = [
                previous_record[name] + value
                for value in predicted_response_quantiles[name]
            ]
        ...
        for name in PHYSICAL_NODES:
            result[f"actual_{name}"] = actual_tail_values[name]
            result[f"pred_{name}"] = predicted_tail_values[name]
            result[f"pred_{name}_p10"] = predicted_tail_quantiles[name][0]
            result[f"pred_{name}_p50"] = predicted_tail_quantiles[name][1]
            result[f"pred_{name}_p90"] = predicted_tail_quantiles[name][2]
            result[f"actual_delta_{name}"] = actual_response[name]
            result[f"pred_delta_{name}"] = predicted_response[name]
            result[f"pred_delta_{name}_p10"] = predicted_response_quantiles[name][0]
            result[f"pred_delta_{name}_p50"] = predicted_response_quantiles[name][1]
            result[f"pred_delta_{name}_p90"] = predicted_response_quantiles[name][2]
        return result
...
def parse_args():
    parser = argparse.ArgumentParser(description="Online rolling quantile GAT-GRU IDS without temperature.")
...
def main():
    ...
    if checkpoint.get("model_type") != "rolling_quantile_gat_gru_delta_predictor":
        raise SystemExit("This script expects a model trained by train_rolling_quantile_gnn_gru_ids.py")
    ...
    print(f"[INFO] Error method: tail_median_response_quantile_interval start_step={response_tail_start(ids.horizon) + 1}")
    print(f"[INFO] Quantiles: {checkpoint.get('quantiles', QUANTILES)}")
